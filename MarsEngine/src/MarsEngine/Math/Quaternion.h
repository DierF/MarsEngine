...
namespace Math
{
    ...
    public:
        ...
        explicit Quaternion(Mat3 const & rot) { this->fromRotationMatrix(rot); }
        /// Construct a quaternion from an angle/axis
        Quaternion(const Radian & angle, Vec3 const & axis) { this->fromAngleAxis(angle, axis); }
        /// Construct a quaternion from 3 orthonormal local axes
        Quaternion(Vec3 const & xaxis, Vec3 const & yaxis, Vec3 const & zaxis)
        {
        ...
        void toRotationMatrix(Mat4 & rotation) const;

        void fromAngleAxis(const Radian & angle, Vec3 const & axis);

        static Quaternion getQuaternionFromAngleAxis(const Radian & angle, Vec3 const & axis);

        void fromDirection(Vec3 const & direction, Vec3 const & up_direction);
        ...
        */
        ...
        void toAxes(Vec3 & x_axis, Vec3 & y_axis, Vec3 & z_axis) const;
        ...
        */
        ...
        Quaternion mul(Quaternion const& rhs) const { return (*this) * rhs; }
        Quaternion operator*(Quaternion const& rhs) const;
        ...
        friend Quaternion operator*(float scalar, Quaternion const& rhs)
        {
        ...
        float dot(Quaternion const& rkQ) const { return w * rkQ.w + x * rkQ.x + y * rkQ.y + z * rkQ.z; }
        ...
        */
        static Quaternion sLerp(float t, Quaternion const& kp, Quaternion const& kq, bool shortest_path = false);
        ...
        /** Performs Normalised linear interpolation between two quaternions, and returns the result.
            ...
            */
        static Quaternion nLerp(float t, Quaternion const& kp, Quaternion const& kq, bool shortest_path = false);
