#include "Editor/Include/EditorFileService.h"
...
namespace MarsEngine
{
    ...
    splitString(std::string input_string, std::string const& separator, std::string const& filter_string = "")
    {
    ...
    void EditorFileService::buildEngineFileTree()
    {
        ...
        std::vector<std::filesystem::path> const file_paths = g_runtime_global_context.m_file_system->getFiles(asset_folder);
        std::vector<std::vector<std::string>>    all_file_segments;
        for (auto const& path : file_paths)
        {
            std::filesystem::path const& relative_path = Path::getRelativePath(asset_folder, path);
        ...
        *root_node     = m_root_node;
        ...
        for (int file_index = 0; file_index < all_file_segments_count; file_index++)
        {
            ...
            for (int file_segment_index = 0; file_segment_index < file_segment_count; file_segment_index++)
            {
                ...
                {
                    auto const& extensions = Path::getFileExtensions(file_paths[file_index]);
