...
namespace MarsEngine
{
    ...
    EditorUI::EditorUI()
    {
        auto const& asset_folder            = g_runtime_global_context.m_config_manager->getAssetFolder();
        m_editor_ui_creator["TreeNodePush"] = [this](std::string const& name, void* value_ptr) -> void
        {
        ...
        m_editor_ui_creator["TreeNodePop"] = [this](std::string const& name, void* value_ptr) -> void
        {
        ...
        m_editor_ui_creator["Transform"] = [this](std::string const& name, void* value_ptr) -> void
        {
            if (g_editor_node_state_array[g_node_depth].second)
            {
        ...
        m_editor_ui_creator["bool"] = [this](std::string const& name, void* value_ptr)  -> void
        {
            if(g_node_depth == -1)
            {
        ...
        m_editor_ui_creator["int"] = [this](std::string const& name, void* value_ptr) -> void
        {
            if (g_node_depth == -1)
            {
                std::string label = "##" + name;
        ...
        m_editor_ui_creator["float"] = [this](std::string const& name, void* value_ptr) -> void
        {
            if (g_node_depth == -1)
            {
                std::string label = "##" + name;
        ...
        m_editor_ui_creator["std::string"] = [this, &asset_folder](std::string const& name, void* value_ptr) -> void
        {
            if (g_node_depth == -1)
            {
                std::string label = "##" + name;
    ...
    void EditorUI::showEditorWorldObjectsWindow(bool* p_open)
    {
        ...
        LevelObjectsMap const& all_gobjects = current_active_level->getAllGObjects();
        for (auto& id_object_pair : all_gobjects)
        {
            GObjectID const          object_id = id_object_pair.first;
    ...
    void EditorUI::showEditorDetailWindow(bool* p_open)
    {
        ...
        std::string const& name = selected_object->getName();
    ...
    void EditorUI::drawAxisToggleButton(char const* string_id, bool check_state, int axis_mode)
    {
        if (check_state)
        {
    ...
    void EditorUI::buildEditorFileAssetsUITree(EditorFileNode* node)
    {
        ...
        bool const is_folder = (node->m_child_nodes.size() > 0);
        if (is_folder)
        {
    ...
    void EditorUI::onFileContentItemClicked(EditorFileNode* node)
    {
        ...
        unsigned int const new_object_index = ++m_new_object_index_map[node->m_file_name];
