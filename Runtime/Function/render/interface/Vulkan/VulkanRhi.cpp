...
namespace MarsEngine
{
    ...
    void VulkanRHI::initialize(RHIInitInfo init_info)
    {
        ...
        // https://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html
#if defined(__linux__)
        char const* vk_layer_path = MARS_XSTR(MARS_VK_LAYER_PATH);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
#elif defined(__MACH__)
        // https://developer.apple.com/library/archive/documentation/Porting/Conceptual/PortingUnix/compiling/compiling.html
        char const* vk_layer_path    = MARS_XSTR(MARS_VK_LAYER_PATH);
        char const* vk_icd_filenames = MARS_XSTR(MARS_VK_ICD_FILENAMES);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
...
#error Unknown Compiler
    ...
    bool VulkanRHI::waitForFences(uint32_t fenceCount, RHIFence const* const* pFences, RHIBool32 waitAll, uint64_t timeout)
    {
        //fence
        ...
        {
            auto const& rhi_fence_element = pFences[i];
    ...
    std::vector<char const*> VulkanRHI::getRequiredExtensions()
    {
    ...
    void VulkanRHI::initializePhysicalDevice()
    {
        ...
        else
        {
            ...
            std::sort(ranked_physical_devices.begin(),
                      ...
                      [](std::pair<int, VkPhysicalDevice> const& p1, std::pair<int, VkPhysicalDevice> const& p2)
                      {
                          return p1 > p2;
                      });

            for (auto const& device : ranked_physical_devices)
            {
    ...
    bool VulkanRHI::createCommandPool(RHICommandPoolCreateInfo const* pCreateInfo, RHICommandPool*& pCommandPool)
    {
        VkCommandPoolCreateInfo create_info{};
        create_info.sType = (VkStructureType)pCreateInfo->sType;
        create_info.pNext = (void const*)pCreateInfo->pNext;
    ...
    bool VulkanRHI::createDescriptorPool(RHIDescriptorPoolCreateInfo const* pCreateInfo, RHIDescriptorPool*&  pDescriptorPool)
    {
        int size = pCreateInfo->poolSizeCount;
        std::vector<VkDescriptorPoolSize> descriptor_pool_size(size);
        for (int i = 0; i < size; ++i)
        {
            auto const& rhi_desc = pCreateInfo->pPoolSizes[i];
    ...
    bool VulkanRHI::createDescriptorSetLayout(RHIDescriptorSetLayoutCreateInfo const* pCreateInfo, RHIDescriptorSetLayout*& pSetLayout)
    {
        ...
        {
            auto const& rhi_descriptor_set_layout_binding_element = pCreateInfo->pBindings[i];
        ...
        for (int i = 0; i < descriptor_set_layout_binding_size; ++i)
        {
            ...
            if (rhi_descriptor_set_layout_binding_element.pImmutableSamplers)
            {
                ...
                {
                    auto const& rhi_sampler_element = rhi_descriptor_set_layout_binding_element.pImmutableSamplers[i];
    ...
    bool VulkanRHI::createFence(RHIFenceCreateInfo const* pCreateInfo, RHIFence*& pFence)
    {
    ...
    bool VulkanRHI::createFramebuffer(RHIFramebufferCreateInfo const* pCreateInfo, RHIFramebuffer*& pFramebuffer)
    {
        //image_view
        ...
        {
            auto const& rhi_image_view_element = pCreateInfo->pAttachments[i];
    ...
    bool VulkanRHI::createGraphicsPipelines(RHIPipelineCache* pipelineCache, uint32_t createInfoCount, RHIGraphicsPipelineCreateInfo const* pCreateInfo, RHIPipeline*& pPipelines)
    {
        ...
        {
            auto const& rhi_pipeline_shader_stage_create_info_element = pCreateInfo->pStages[i];
        ...
        for (int i = 0; i < pipeline_shader_stage_create_info_size; ++i)
        {
            ...
            if (rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo != nullptr)
            {
                ...
                vk_specialization_info.pData = (void const*)rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo->pData;

                //specialization_map_entry
                for (int i = 0; i < rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo->mapEntryCount; ++i)
                {
                    auto const& rhi_specialization_map_entry_element = rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo->pMapEntries[i];
            ...
            vk_pipeline_shader_stage_create_info_element.pNext = (void const*)rhi_pipeline_shader_stage_create_info_element.pNext;
        ...
        {
            auto const& rhi_vertex_input_binding_description_element = pCreateInfo->pVertexInputState->pVertexBindingDescriptions[i];
        ...
        {
            auto const& rhi_vertex_input_attribute_description_element = pCreateInfo->pVertexInputState->pVertexAttributeDescriptions[i];
        ...
        vk_pipeline_vertex_input_state_create_info.pNext = (void const*)pCreateInfo->pVertexInputState->pNext;
        ...
        vk_pipeline_input_assembly_state_create_info.pNext = (void const*)pCreateInfo->pInputAssemblyState->pNext;
        ...
        VkPipelineTessellationStateCreateInfo const* vk_pipeline_tessellation_state_create_info_ptr = nullptr;
        ...
        if (pCreateInfo->pTessellationState != nullptr)
        {
            ...
            vk_pipeline_tessellation_state_create_info.pNext = (void const*)pCreateInfo->pTessellationState->pNext;
        ...
        {
            auto const& rhi_viewport_element = pCreateInfo->pViewportState->pViewports[i];
        ...
        {
            auto const& rhi_rect_2d_element = pCreateInfo->pViewportState->pScissors[i];
        ...
        vk_pipeline_viewport_state_create_info.pNext = (void const*)pCreateInfo->pViewportState->pNext;
        ...
        vk_pipeline_rasterization_state_create_info.pNext = (void const*)pCreateInfo->pRasterizationState->pNext;
        ...
        vk_pipeline_multisample_state_create_info.pNext = (void const*)pCreateInfo->pMultisampleState->pNext;
        ...
        vk_pipeline_multisample_state_create_info.pSampleMask = (RHISampleMask const*)pCreateInfo->pMultisampleState->pSampleMask;
        ...
        vk_pipeline_depth_stencil_state_create_info.pNext = (void const*)pCreateInfo->pDepthStencilState->pNext;
        ...
        {
            auto const& rhi_pipeline_color_blend_attachment_state_element = pCreateInfo->pColorBlendState->pAttachments[i];
        ...
        {
            auto const& rhi_dynamic_state_element = pCreateInfo->pDynamicState->pDynamicStates[i];
    ...
    bool VulkanRHI::createComputePipelines(RHIPipelineCache* pipelineCache, uint32_t createInfoCount, RHIComputePipelineCreateInfo const* pCreateInfos, RHIPipeline*& pPipelines)
    {
        ...
        shader_stage_create_info.pNext = (void const*)pCreateInfos->pStages->pNext;
        ...
        create_info.pNext = (void const*)pCreateInfos->pNext;
    ...
    bool VulkanRHI::createPipelineLayout(RHIPipelineLayoutCreateInfo const* pCreateInfo, RHIPipelineLayout*& pPipelineLayout)
    {
        ...
        {
            auto const& rhi_descriptor_set_layout_element = pCreateInfo->pSetLayouts[i];
    ...
    bool VulkanRHI::createRenderPass(RHIRenderPassCreateInfo const* pCreateInfo, RHIRenderPass*& pRenderPass)
    {
        // attachment convert
        std::vector<VkAttachmentDescription> vk_attachments(pCreateInfo->attachmentCount);
        for (int i = 0; i < pCreateInfo->attachmentCount; ++i)
        {
            auto const& rhi_desc = pCreateInfo->pAttachments[i];
        ...
        {
            auto const& rhi_desc = pCreateInfo->pSubpasses[i];
        ...
        for (int i = 0; i < pCreateInfo->subpassCount; ++i)
        {
            ...
            vk_desc.pPreserveAttachments = (uint32_t const*)(rhi_desc).pPreserveAttachments;
            ...
            {
                auto const& rhi_attachment_refence_input = (rhi_desc).pInputAttachments[i];
            ...
            {
                auto const& rhi_attachment_refence_color = (rhi_desc).pColorAttachments[i];
            ...
            if (rhi_desc.pResolveAttachments != nullptr)
            {
                ...
                for (int i = 0; i < (rhi_desc).colorAttachmentCount; ++i)
                {
                    auto const& rhi_attachment_refence_resolve = (rhi_desc).pResolveAttachments[i];
            ...
            if (rhi_desc.pDepthStencilAttachment != nullptr)
            {
                ...
                for (int i = 0; i < (rhi_desc).colorAttachmentCount; ++i)
                {
                    auto const& rhi_attachment_refence_depth = (rhi_desc).pDepthStencilAttachment[i];
        ...
        {
            auto const& rhi_desc = pCreateInfo->pDependencies[i];
    ...
    bool VulkanRHI::createSampler(RHISamplerCreateInfo const* pCreateInfo, RHISampler*& pSampler)
    {
    ...
    bool VulkanRHI::beginCommandBufferPFN(RHICommandBuffer* commandBuffer, RHICommandBufferBeginInfo const* pBeginInfo)
    {
        ...
        if (pBeginInfo->pInheritanceInfo != nullptr)
        {
            ...
            command_buffer_inheritance_info.pNext = (void const*)pBeginInfo->pInheritanceInfo->pNext;
        ...
        command_buffer_begin_info.pNext = (void const*)pBeginInfo->pNext;
    ...
    void VulkanRHI::cmdBeginRenderPassPFN(RHICommandBuffer* commandBuffer, RHIRenderPassBeginInfo const* pRenderPassBegin, RHISubpassContents contents)
    {
        ...
        {
            auto const& rhi_clear_value_element = pRenderPassBegin->pClearValues[i];
    ...
    void VulkanRHI::cmdSetViewportPFN(RHICommandBuffer* commandBuffer, uint32_t firstViewport, uint32_t viewportCount, const RHIViewport* pViewports)
    {
        ...
        for (int i = 0; i < viewport_size; ++i)
        {
            auto const& rhi_viewport_element = pViewports[i];
            auto& vk_viewport_element = vk_viewport_list[i];
    ...
    void VulkanRHI::cmdSetScissorPFN(RHICommandBuffer* commandBuffer, uint32_t firstScissor, uint32_t scissorCount, RHIRect2D const* pScissors)
    {
        //rect_2d
        ...
        for (int i = 0; i < rect_2d_size; ++i)
        {
            auto const& rhi_rect_2d_element = pScissors[i];
            auto& vk_rect_2d_element = vk_rect_2d_list[i];
        ...
        RHIDeviceSize const* pOffsets)
    {
        ...
        {
            auto const& rhi_buffer_element = pBuffers[i];
        ...
        {
            auto const& rhi_offset_element = pOffsets[i];
        ...
        uint32_t const* pDynamicOffsets)
    {
        ...
        {
            auto const& rhi_descriptor_set_element = pDescriptorSets[i];
        ...
        for (int i = 0; i < offset_size; ++i)
        {
            auto const& rhi_offset_element = pDynamicOffsets[i];
    ...
    void VulkanRHI::cmdClearAttachmentsPFN(
        ...
        RHIClearAttachment const* pAttachments,
        uint32_t rectCount,
        RHIClearRect const* pRects)
    {
        //clear_attachment
        ...
        {
            auto const& rhi_clear_attachment_element = pAttachments[i];
        ...
        {
            auto const& rhi_clear_rect_element = pRects[i];
    ...
    bool VulkanRHI::beginCommandBuffer(RHICommandBuffer* commandBuffer, RHICommandBufferBeginInfo const* pBeginInfo)
    {
        VkCommandBufferInheritanceInfo command_buffer_inheritance_info{};
        VkCommandBufferInheritanceInfo const* command_buffer_inheritance_info_ptr = nullptr;
        if (pBeginInfo->pInheritanceInfo != nullptr)
        {
            command_buffer_inheritance_info.sType = (VkStructureType)(pBeginInfo->pInheritanceInfo->sType);
            command_buffer_inheritance_info.pNext = (void const*)pBeginInfo->pInheritanceInfo->pNext;
    ...
    void VulkanRHI::updateDescriptorSets(
        ...
        RHIWriteDescriptorSet const* pDescriptorWrites,
        uint32_t descriptorCopyCount,
        RHICopyDescriptorSet const* pDescriptorCopies)
    {
        ...
        {
            auto const& rhi_write_descriptor_set_element = pDescriptorWrites[i];
        ...
        for (int i = 0; i < write_descriptor_set_size; ++i)
        {
            ...
            VkDescriptorBufferInfo const* vk_descriptor_buffer_info_ptr = nullptr;
            if (rhi_write_descriptor_set_element.pBufferInfo != nullptr)
            {
            ...
            vk_write_descriptor_set_element.pNext = (void const*)rhi_write_descriptor_set_element.pNext;
        ...
        {
            auto const& rhi_copy_descriptor_set_element = pDescriptorCopies[i];
            ...
            vk_copy_descriptor_set_element.pNext = (void const*)rhi_copy_descriptor_set_element.pNext;
    ...
    bool VulkanRHI::queueSubmit(RHIQueue* queue, uint32_t submitCount, RHISubmitInfo const* pSubmits, RHIFence* fence)
    {
        //submit_info
        ...
        {
            auto const& rhi_submit_info_element = pSubmits[i];
        ...
        for (int i = 0; i < submit_info_size; ++i)
        {
            ...
            vk_submit_info_element.pNext = (void const*)rhi_submit_info_element.pNext;
            ...
            if (rhi_submit_info_element.commandBufferCount > 0)
            {
                ...
                {
                    auto const& rhi_command_buffer_element = rhi_submit_info_element.pCommandBuffers[i];
            ...
            if (rhi_submit_info_element.waitSemaphoreCount > 0)
            {
                ...
                {
                    auto const& rhi_semaphore_element = rhi_submit_info_element.pWaitSemaphores[i];
            ...
            if (rhi_submit_info_element.signalSemaphoreCount > 0)
            {
                ...
                {
                    auto const& rhi_signal_semaphore_element = rhi_submit_info_element.pSignalSemaphores[i];
            ...
            if (rhi_submit_info_element.waitSemaphoreCount > 0)
            {
                ...
                {
                    auto const& rhi_pipeline_stage_flags_element = rhi_submit_info_element.pWaitDstStageMask[i];
    ...
    void VulkanRHI::cmdPipelineBarrier(RHICommandBuffer* commandBuffer,
        ...
        RHIMemoryBarrier const* pMemoryBarriers,
        uint32_t bufferMemoryBarrierCount,
        RHIBufferMemoryBarrier const* pBufferMemoryBarriers,
        uint32_t imageMemoryBarrierCount,
        RHIImageMemoryBarrier const* pImageMemoryBarriers)
    {
        //memory_barrier
        ...
        {
            auto const& rhi_memory_barrier_element = pMemoryBarriers[i];
            ...
            vk_memory_barrier_element.pNext = (void const*)rhi_memory_barrier_element.pNext;
        ...
        {
            auto const& rhi_buffer_memory_barrier_element = pBufferMemoryBarriers[i];
            ...
            vk_buffer_memory_barrier_element.pNext = (void const*)rhi_buffer_memory_barrier_element.pNext;
        ...
        {
            auto const& rhi_image_memory_barrier_element = pImageMemoryBarriers[i];
            ...
            vk_image_memory_barrier_element.pNext = (void const*)rhi_image_memory_barrier_element.pNext;
    ...
    void VulkanRHI::cmdCopyImageToBuffer(
        ...
        RHIBufferImageCopy const* pRegions)
    {
        //buffer_image_copy
        ...
        {
            auto const& rhi_buffer_image_copy_element = pRegions[i];
    ...
    RHIShader* VulkanRHI::createShaderModule(std::vector<unsigned char> const& shader_code)
    {
    ...
    bool VulkanRHI::createBufferVMA(VmaAllocator allocator, RHIBufferCreateInfo const* pBufferCreateInfo, VmaAllocationCreateInfo const* pAllocationCreateInfo, RHIBuffer*&  pBuffer, VmaAllocation* pAllocation, VmaAllocationInfo* pAllocationInfo)
    {
        VkBuffer vk_buffer;
        VkBufferCreateInfo buffer_create_info{};
        buffer_create_info.sType = (VkStructureType)pBufferCreateInfo->sType;
        buffer_create_info.pNext = (void const*)pBufferCreateInfo->pNext;
        ...
        buffer_create_info.pQueueFamilyIndices = (uint32_t const*)pBufferCreateInfo->pQueueFamilyIndices;
    ...
    bool VulkanRHI::createBufferWithAlignmentVMA(VmaAllocator allocator, RHIBufferCreateInfo const* pBufferCreateInfo, const VmaAllocationCreateInfo* pAllocationCreateInfo, RHIDeviceSize minAlignment, RHIBuffer*& pBuffer, VmaAllocation* pAllocation, VmaAllocationInfo* pAllocationInfo)
    {
        VkBuffer vk_buffer;
        VkBufferCreateInfo buffer_create_info{};
        buffer_create_info.sType = (VkStructureType)pBufferCreateInfo->sType;
    ...
    bool VulkanRHI::allocateDescriptorSets(RHIDescriptorSetAllocateInfo const* pAllocateInfo, RHIDescriptorSet*& pDescriptorSets)
    {
        //descriptor_set_layout
        ...
        for (int i = 0; i < descriptor_set_layout_size; ++i)
        {
            auto const& rhi_descriptor_set_layout_element = pAllocateInfo->pSetLayouts[i];
            auto& vk_descriptor_set_layout_element = vk_descriptor_set_layout_list[i];
        ...
        descriptorset_allocate_info.pNext = (void const*)pAllocateInfo->pNext;
    ...
    bool VulkanRHI::allocateCommandBuffers(RHICommandBufferAllocateInfo const* pAllocateInfo, RHICommandBuffer*& pCommandBuffers)
    {
        VkCommandBufferAllocateInfo command_buffer_allocate_info{};
        command_buffer_allocate_info.sType = (VkStructureType)pAllocateInfo->sType;
        command_buffer_allocate_info.pNext = (void const*)pAllocateInfo->pNext;
    ...
    VkFormat VulkanRHI::findSupportedFormat(std::vector<VkFormat> const& candidates,
    ...
    VkSurfaceFormatKHR
    VulkanRHI::chooseSwapchainSurfaceFormatFromDetails(std::vector<VkSurfaceFormatKHR> const& available_surface_formats)
    {
        for (auto const& surface_format : available_surface_formats)
        {
    ...
    VkPresentModeKHR
    VulkanRHI::chooseSwapchainPresentModeFromDetails(std::vector<VkPresentModeKHR> const& available_present_modes)
    {
    ...
    VkExtent2D VulkanRHI::chooseSwapchainExtentFromDetails(VkSurfaceCapabilitiesKHR const& capabilities)
    {
    ...
    void VulkanRHI::pushEvent(RHICommandBuffer* commond_buffer, char const* name, float const* color)
    {
        if (m_enable_debug_utils_label)
        {
