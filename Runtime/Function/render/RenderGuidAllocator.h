...
{
    static size_t const s_invalid_guid = 0;
    ...
    public:
        ...
        bool getElementGuid(T const& t, size_t& guid)
        {
            auto find_it = m_elements_guid_map.find(t);
            if (find_it != m_elements_guid_map.end())
            {
        ...
        bool hasElement(T const& t) { return m_elements_guid_map.find(t) != m_elements_guid_map.end(); }
        ...
        void freeGuid(size_t guid)
        {
            ...
            if (find_it != m_guid_elements_map.end())
            {
                auto const& ele = find_it->second;
                m_elements_guid_map.erase(ele);
                m_guid_elements_map.erase(guid);
            }
        }

        void freeElement(T const& t)
        {
            auto find_it = m_elements_guid_map.find(t);
            if (find_it != m_elements_guid_map.end())
            {
                auto const& guid = find_it->second;
        ...
        std::vector<size_t> getAllocatedGuids() const
        {
            ...
            for (auto const& ele : m_guid_elements_map)
            {
