...
namespace MarsEngine
{
    ...
    void RenderSystem::initialize(RenderSystemInitInfo init_info)
    {
        ...
        std::string const& global_rendering_res_url = config_manager->getGlobalRenderingResUrl();
        ...
        // setup render camera
        CameraPose const& camera_pose = global_rendering_res.m_camera_config.m_pose;
        ...
        m_render_scene->m_ambient_light = {global_rendering_res.m_ambient_light.toVec3()};
        m_render_scene->m_directional_light.m_direction =
            global_rendering_res.m_directional_light.m_direction.normalisedCopy();
        m_render_scene->m_directional_light.m_color = global_rendering_res.m_directional_light.m_color.toVec3();
    ...
    void RenderSystem::processSwapData()
    {
        ...
        if (swap_data.m_game_object_resource_desc.has_value())
        {
            while (!swap_data.m_game_object_resource_desc->isEmpty())
            {
                ...
                {
                    auto const&      game_object_part = gobject.getObjectParts()[part_index];
        ...
        if (swap_data.m_particle_submit_request.has_value())
        {
            ...
            {
                ParticleEmitterDesc const& desc = swap_data.m_particle_submit_request->getEmitterDesc(index);
