#include "Runtime/Function/Render/DebugDraw/DebugDrawBuffer.h"
...
namespace MarsEngine
{
    ...
    size_t DebugDrawAllocator::cacheVertexs(std::vector<DebugDrawVertex> const& vertexs)
    {
    ...
    void DebugDrawAllocator::allocator()
    {
        ...
        uint64_t uniform_dynamic_BufferSize = static_cast<uint64_t>(sizeof(UniformBufferDynamicObject)
            * m_uniform_buffer_dynamic_object_cache.size());
    ...
    size_t const DebugDrawAllocator::getSphereVertexBufferSize() const
    {
        return ((m_circle_sample_count * 2 + 2) * (m_circle_sample_count * 2) * 2 + (m_circle_sample_count * 2 + 1) * (m_circle_sample_count * 2) * 2);
    }

    size_t const DebugDrawAllocator::getCylinderVertexBufferSize() const
    {
        return (m_circle_sample_count * 2) * 5 * 2;
    }

    size_t const DebugDrawAllocator::getCapsuleVertexBufferSize() const
    {
    ...
    size_t const DebugDrawAllocator::getCapsuleVertexBufferUpSize() const
    {
        return (m_circle_sample_count * 2) * m_circle_sample_count * 4;
    }

    size_t const DebugDrawAllocator::getCapsuleVertexBufferMidSize() const
    {
        return 2 * m_circle_sample_count * 2;
    }

    size_t const DebugDrawAllocator::getCapsuleVertexBufferDownSize() const
    {
        return 2 * m_circle_sample_count * m_circle_sample_count * 4;
    }

    size_t const DebugDrawAllocator::getSizeOfUniformBufferObject() const
    {
