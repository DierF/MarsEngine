...
namespace MarsEngine
{
    ...
    void DebugDrawGroup::setName(std::string const& name) { m_name = name; }
    ...
    {
        size_t count = 0;
        for (DebugDrawPoint const point : m_points)
        {
    ...
    {
        size_t line_count = 0;
        for (DebugDrawLine const line : m_lines)
        {
            if (line.m_no_depth_test == no_depth_test)line_count++;
        }
        for (DebugDrawTriangle const triangle : m_triangles)
        {
        ...
        for (DebugDrawQuad const quad : m_quads)
        {
        ...
        for (DebugDrawBox const box : m_boxes)
        {
    ...
    {
        size_t count = 0;
        for (DebugDrawSphere const sphere : m_spheres)
        {
    ...
    {
        size_t count = 0;
        for (DebugDrawCylinder const cylinder : m_cylinders)
        {
    ...
    {
        size_t count = 0;
        for (DebugDrawCapsule const capsule : m_capsules)
        {
    ...
    {
        size_t count = 0;
        for (DebugDrawText const text : m_texts)
        {
            for (unsigned char character : text.m_content)
            {
                if (character != '\n')count++;
