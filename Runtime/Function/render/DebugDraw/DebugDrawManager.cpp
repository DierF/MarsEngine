...
namespace MarsEngine
{
    ...
    {
        RenderResource const* resource = static_cast<RenderResource const*>(render_resource.get());
    ...
    DebugDrawGroup* DebugDrawManager::tryGetOrCreateDebugDrawGroup(std::string const& name)
    {
        std::lock_guard<std::mutex> guard(m_mutex);
