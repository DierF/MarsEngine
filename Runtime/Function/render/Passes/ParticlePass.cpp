...
namespace MarsEngine
{
    ...
    void ParticlePass::createEmitter(int id, ParticleEmitterDesc const& desc)
    {
        VkDeviceSize const counterBufferSize = sizeof(ParticleCounter);
        ...
        {
            VkDeviceSize const      indirectArgumentSize = sizeof(IndirectArgumemt);
            ...
            VkDeviceSize const aliveListSize = 4 * sizeof(uint32_t) * s_max_particles;
            ...
            VkDeviceSize const   deadListSize = 4 * sizeof(uint32_t) * s_max_particles;
        ...
        VkDeviceSize const staggingBuferSize        = s_max_particles * sizeof(Particle);
    ...
    void ParticlePass::initialize(RenderPassInitInfo const* init_info)
    {
        RenderPass::initialize(nullptr);

        ParticlePassInitInfo const* _init_info = static_cast<ParticlePassInitInfo const*>(init_info);
    ...
    void ParticlePass::simulate()
    {
        for (auto i : m_emitter_tick_indices)
        {
            ...
            m_rhi->resetFencesPFN(1, &m_fence);
            computeSubmitInfo                        = {};
            VkPipelineStageFlags const waitStageMask = RHI_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
            computeSubmitInfo.sType                  = RHI_STRUCTURE_TYPE_SUBMIT_INFO;
    ...
    void ParticlePass::prepareUniformBuffer()
    {
        ...
        GlobalParticleRes const& global_res = m_particle_manager->getGlobalParticleRes();
    ...
    {
        RenderResource const* vulkan_resource = static_cast<RenderResource const*>(render_resource.get());
        if (vulkan_resource)
        {
    ...
    void ParticlePass::setTickIndices(std::vector<ParticleEmitterID> const& tick_indices)
    {
        m_emitter_tick_indices = tick_indices;
    }

    void ParticlePass::setTransformIndices(std::vector<ParticleEmitterTransformDesc> const& transform_indices)
    {
