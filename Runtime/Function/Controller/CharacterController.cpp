...
namespace MarsEngine
{
    CharacterController::CharacterController(Capsule const& capsule) : m_capsule(capsule)
    {
        ...
        *static_cast<Capsule*>(m_rigidbody_shape.m_geometry) = m_capsule;
        ...
        orientation.fromAngleAxis(Radian(Degree(90.f)), Vec3::UNIT_X);

        m_rigidbody_shape.m_local_transform =
            Transform(Vec3(0, 0, capsule.m_half_height + capsule.m_radius), orientation, Vec3::UNIT_SCALE);
    }

    Vec3 CharacterController::move(Vec3 const& current_position, Vec3 const& displacement)
    {
        std::shared_ptr<PhysicsScene> physics_scene =
            g_runtime_global_context.m_world_manager->getCurrentActivePhysicsScene().lock();
        ASSERT(physics_scene);

        Vec3 final_position = current_position + displacement;

        Transform final_transform = Transform(final_position, Quaternion::IDENTITY, Vec3::UNIT_SCALE);
