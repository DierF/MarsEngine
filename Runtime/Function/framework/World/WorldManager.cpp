...
namespace MarsEngine
{
    ...
    bool WorldManager::loadWorld(std::string const& world_url)
    {
        LOG_INFO("loading world: {}", world_url);
        WorldRes   world_res;
        bool const is_world_load_success = g_runtime_global_context.m_asset_manager->loadAsset(world_url, world_res);
        ...
        bool const is_level_load_success = loadLevel(world_res.m_default_level_url);
    ...
    bool WorldManager::loadLevel(std::string const& level_url)
    {
        std::shared_ptr<Level> level = std::make_shared<Level>();
        // set current level temporary
        m_current_active_level       = level;

        bool const is_level_load_success = level->load(level_url);
    ...
    void WorldManager::reloadCurrentLevel()
    {
        ...
        std::string const level_url = active_level->getLevelResUrl();
        active_level->unload();
        m_loaded_levels.erase(level_url);

        bool const is_load_success = loadLevel(level_url);
        if (!is_load_success)
        {
