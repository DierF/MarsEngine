...
{
    char const* k_unknown_type = "UnknownType";
    char const* k_unknown      = "Unknown";
    ...
    void TypeMetaRegisterinterface::registerToFieldMap(char const* name, FieldFunctionTuple* value)
    {
        m_field_map.insert(std::make_pair(name, value));
    }
    void TypeMetaRegisterinterface::registerToMethodMap(char const* name, MethodFunctionTuple* value)
    {
        m_method_map.insert(std::make_pair(name, value));
    }
    void TypeMetaRegisterinterface::registerToArrayMap(char const* name, ArrayFunctionTuple* value)
    {
    ...
    void TypeMetaRegisterinterface::registerToClassMap(char const* name, ClassFunctionTuple* value)
    {
    ...
    {
        for (auto const& itr : m_field_map)
        {
            delete itr.second;
        }
        m_field_map.clear();
        for (auto const& itr : m_class_map)
        {
            delete itr.second;
        }
        m_class_map.clear();
        for (auto const& itr : m_array_map)
        {
            delete itr.second;
        }
        m_array_map.clear();
    ...
    ReflectionInstance TypeMeta::newFromNameAndJson(std::string type_name, Json const& json_context)
    {
    ...
    FieldAccessor TypeMeta::getFieldByName(char const* name)
    {
        auto const it = std::find_if(m_fields.begin(), m_fields.end(), [&](auto const& i) {
    ...
    MethodAccessor TypeMeta::getMethodByName(char const* name)
    {
        auto const it = std::find_if(m_methods.begin(), m_methods.end(), [&](auto const& i) {
    ...
    TypeMeta& TypeMeta::operator=(TypeMeta const& dest)
    {
        if (this == &dest)
        {
            return *this;
    ...
    char const* FieldAccessor::getFieldName() const { return m_field_name; }
    char const* FieldAccessor::getFieldTypeName() { return m_field_type_name; }
    ...
    FieldAccessor& FieldAccessor::operator=(FieldAccessor const& dest)
    {
        if (this == &dest)
        {
            return *this;
    ...
    char const* MethodAccessor::getMethodName() const{
        return (std::get<0>(*m_functions))();
    }
    MethodAccessor& MethodAccessor::operator=(MethodAccessor const& dest)
    {
        if (this == &dest)
        {
            return *this;
    ...
    char const* ArrayAccessor::getArrayTypeName() { return m_array_type_name; }
    char const* ArrayAccessor::getElementTypeName() { return m_element_type_name; }
