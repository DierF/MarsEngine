...
#define MARS_REFLECTION_NEW(name, ...) Reflection::ReflectionPtr(#name, new name(__VA_ARGS__));
#define MARS_REFLECTION_DELETE(value) \
    if (value) \
    { \
        delete value.operator->(); \
        value.getPtrReference() = nullptr; \
    }
#define MARS_REFLECTION_DEEP_COPY(type, dst_ptr, src_ptr) \
    *static_cast<type*>(dst_ptr) = *static_cast<type*>(src_ptr.getPtr());

#define TypeMetaDef(class_name, ptr) \
    MarsEngine::Reflection::ReflectionInstance(MarsEngine::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                            (class_name*)ptr)

#define TypeMetaDefPtr(class_name, ptr) \
    new MarsEngine::Reflection::ReflectionInstance(MarsEngine::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                                (class_name*)ptr)
    ...
    typedef std::function<char const*()>           GetNameFuncion;
    ...
    typedef std::function<void*(Json const&)>                           ConstructorWithJson;
    ...
    namespace Reflection
    {
        ...
        {
        public:
            static void registerToClassMap(char const* name, ClassFunctionTuple* value);
            static void registerToFieldMap(char const* name, FieldFunctionTuple* value);

            static void registerToMethodMap(char const* name, MethodFunctionTuple* value);
            static void registerToArrayMap(char const* name, ArrayFunctionTuple* value);
        ...
        public:
            ...
            static ReflectionInstance newFromNameAndJson(std::string type_name, Json const& json_context);
            ...
            FieldAccessor getFieldByName(char const* name);
            MethodAccessor getMethodByName(char const* name);

            bool isValid() { return m_is_valid; }

            TypeMeta& operator=(TypeMeta const& dest);
        ...
        public:
            ...
            char const* getFieldName() const;
            char const* getFieldTypeName();
            bool        isArrayType();

            FieldAccessor& operator=(FieldAccessor const& dest);
        ...
        private:
            ...
            char const*         m_field_name;
            char const*         m_field_type_name;
        };
        class MethodAccessor
        {
            friend class TypeMeta;
        ...
        public:
            ...
            MethodAccessor& operator=(MethodAccessor const& dest);
        ...
        public:
            ArrayAccessor();
            char const* getArrayTypeName();
            char const* getElementTypeName();
        ...
        private:
            ...
            char const*         m_array_type_name;
            char const*         m_element_type_name;
        };
        ...
        public:
            ...
            template<
                typename T1 /*, typename = typename std::enable_if<std::is_safely_castable<T*, T1*>::value>::type*/>
            explicit operator T1 const*() const
            {
                return static_cast<T1*>(m_instance);
            }
            ...
            T const& operator*() const { return *(static_cast<T const*>(m_instance)); }
