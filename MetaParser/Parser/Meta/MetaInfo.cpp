...
MetaInfo::MetaInfo(Cursor const& cursor)
{
...
std::string MetaInfo::getProperty(std::string const& key) const
{
...
bool MetaInfo::getFlag(std::string const& key) const { return m_properties.find(key) != m_properties.end(); }

std::vector<MetaInfo::Property> MetaInfo::extractProperties(Cursor const& cursor) const
{
    ...
    static std::string const white_space_string = " \t\r\n";
