...
BaseClass::BaseClass(Cursor const& cursor) : name(Utils::getTypeNameWithoutNamespace(cursor.getType())) {}

Class::Class(Cursor const& cursor, Namespace const& current_namespace) :
    ...
    m_display_name(Utils::getNameWithoutFirstM(m_qualified_name))
{
    ...
    Utils::replaceAll(m_name, "MarsEngine::", "");
